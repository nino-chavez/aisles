...
| 4 — Kibble routes | Home, product listing, product detail, search, error, and cart surfaces render Preserve. A server-gated sandbox adapter adds optionless one-time cart operations and hosted-checkout handoff without changing model authority; committed deployments keep it off. Account, orders, Auto-Refill, and the canonical checkout phase routes remain unavailable; bare `/checkout` remains the source 404. | Pass durable-session and release gates, then complete route-by-route human visual review; do not treat the minimum cart slice as complete commerce |
...
| E | `aisles`: Kibble route and renderer integration | Home, product listing, catalog presentation, search, errors, and a server-gated sandbox cart are integrated in Preserve. Committed deployments keep cart operations off; account, orders, Auto-Refill, and the three canonical checkout phase routes remain unavailable; bare `/checkout` remains the source 404. |
