...
The invariant proves structural validity inside Aisles's current component vocabulary. It does not prove fidelity to a merchant's visual system, page recipes, responsive behavior, or commerce behavior. The repository now has strict organization/brand/route/surface/zone authority and a contract-bound Kibble Preserve path for Home, product listing, search, and error surfaces. Product detail remains development review only. Cart, account, subscriptions, and the three canonical checkout phase routes are source-native unavailable shells, not functional commerce; the bare `/checkout` path remains the canonical source's 404. The contract records 28 zone families as 36 exact identities; 11 use content-backed Kibble-native adapters and 25 are Trusted Hidden. The Kibble 15-route × three-viewport harness and Bealls 90-cell internal harness are mechanical evidence only. Final cold-review repairs, route-by-route human visual approval, PDP publication, runtime policy writes/audit storage, migration/deploy/live database-provider verification, and Bealls external-reference contracts remain required for a reference-preservation claim. See [the autonomy plan](organization-brand-autonomy-plan.md) and [the Kibble boundary retrospective](retrospective-kibble-reference-boundary.md).
