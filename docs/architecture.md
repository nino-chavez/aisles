...
The invariant proves structural validity inside Aisles's current component vocabulary. It does not prove fidelity to a merchant's visual system, page recipes, responsive behavior, or commerce behavior. The repository now has strict organization/brand/route/surface/zone authority and a contract-bound Kibble Preserve path for Home, product listing, catalog presentation, search, error, and cart surfaces. A server-gated BigCommerce sandbox slice supports optionless one-time add, cart read/update/remove/empty, and hosted-checkout handoff; every committed deployment keeps that slice off until durable session storage passes. Account, orders, Auto-Refill, and the three canonical checkout phase routes remain unavailable, and bare `/checkout` remains the canonical source's 404. The contract records 28 zone families as 36 exact identities; 11 use content-backed Kibble-native adapters and 25 are Trusted Hidden. The Kibble 15-route × three-viewport harness and Bealls 90-cell internal harness are mechanical evidence only. Route-by-route human visual approval, runtime policy writes/audit storage, deployment/live database-provider verification, and Bealls external-reference contracts remain required for a reference-preservation claim. See [the autonomy plan](organization-brand-autonomy-plan.md) and [the Kibble boundary retrospective](retrospective-kibble-reference-boundary.md).
