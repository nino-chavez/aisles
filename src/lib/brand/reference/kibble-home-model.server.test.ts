...
describe('bounded Kibble Home model prompt', () => {
	it('contains only approved facts and merchant presentation IDs', () => {
		const prompt = buildKibbleHomeModelPrompt(inference, [{
			...
			catalogSignals: { subscriptionEligible: true, subscriptionCapabilities: ['subscribe-and-save', 'intro-offer'], subscriptionSavingsPercent: 15, subscriptionCadenceMonths: [1, 2, 3] },
		}], {
		...
		expect(prompt).toContain('subscription: subscribe-and-save, intro-offer | save 15% | cadence 1/2/3');
