...
<div class="kibble-reference kc-reference-status" aria-label="Auto-Refill service status">
...
<header class:kc-reference-header--searching={searchOpen} class="kibble-reference kc-reference-header">
		<div class="kc-reference-container kc-reference-header__inner">
			...
			<nav class="kc-reference-header__nav" aria-label="Catalog">
				{#each navItems as item (item.href)}
					<a href={item.href} class="kc-reference-header__nav-link kc-reference-focus">{item.label}</a>
				{:else}
					<span class="kc-reference-header__nav-link">Catalog loading…</span>
				{/each}
			</nav>
			...
			<div class="kc-reference-header__controls">
				{#if searchOpen}
					<form method="GET" action={searchAction} class="kc-reference-search" role="search">
						<label class="sr-only" for="kibble-reference-search">Search products</label>
						<input id="kibble-reference-search" name="q" type="search" placeholder="Search products…" class="kc-reference-focus" />
				...
				{:else}
					<button type="button" onclick={() => (searchOpen = true)} class="kc-reference-text-control kc-reference-focus" aria-label="Search products">
						...
						<span class="kc-reference-header__search-label" style="margin-left:0.4rem;">Search</span>
					</button>
				{/if}
				...
				{#if onPicksClick}
					<div class="kc-reference-control-wrap kc-reference-header__picks">
						<button type="button" onclick={onPicksClick} class="kc-reference-icon-control kc-reference-focus" aria-label="Saved picks ({picksCount})">
				...
				{:else if picksHref}
					<div class="kc-reference-control-wrap kc-reference-header__picks">
						<a href={picksHref} class="kc-reference-icon-control kc-reference-focus" aria-label="Saved picks ({picksCount})">
							<svg aria-hidden="true" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6"><path d="m12 21-1.45-1.32C5.4 15 2 11.92 2 8.14 2 5.06 4.42 3 7.5 3A6 6 0 0 1 12 5.09 6 6 0 0 1 16.5 3C19.58 3 22 5.06 22 8.14c0 3.78-3.4 6.86-8.55 11.54Z"/></svg>
						</a>
						{#if picksCount > 0}<span class="kc-reference-control-badge">{picksCount}</span>{/if}
					</div>
