...
Related-brand reuse inside one integrated merchant organization is a separate case from preserving an unrelated merchant's existing reference storefront. The repository now contains a contract-bound Kibble Preserve path for the home, product-listing, product-detail, and error surfaces. Product listing includes canonical breadcrumbs, seven trusted sort controls, 24-item cursor pagination, and a fixed Kibble shell. Product detail is available only for development review; publication still requires visual and human approval. This is a local implementation under acceptance review, not a deployed or complete reference-preservation claim. Kibble search, cart, and checkout remain outside that contract and fail closed. See the [organization, brand, and composition autonomy plan](docs/organization-brand-autonomy-plan.md) and the [Kibble boundary retrospective](docs/retrospective-kibble-reference-boundary.md).
